...
<<<<<<< HEAD
        ...
        try:
            # initialize may block / raise; do best-effort here
            self.client.initialize()
            self._client_ready = True
        except Exception as ex:
            logging.warning(f"MASsoft client initialize failed: {ex}")
...
<<<<<<< HEAD
            try:
                ...
                logging.error(f"Failed to open experiment {fname}: {ex}")
                self._client_ready = False
=======
...
<<<<<<< HEAD
            # 1) Get and parse legends → mass PVs (with retries)
            headers = None
            for attempt in range(3):
                try:
                    ...
                    logging.error(f"Error fetching legends (attempt {attempt+1}): {ex}")
                    # try to reinitialize client once
                    try:
                        await loop.run_in_executor(None, self.client.initialize)
                        self._client_ready = True
                    except Exception as ex2:
                        logging.warning(f"Re-init failed: {ex2}")
                    await asyncio.sleep(1.0)

            if not headers:
                logging.error("Could not retrieve legends; aborting acquisition loop")
                self._running = False
                return

            try:
                mass_vals = [float(h.split()[-1]) for h in headers if 'mass' in h.lower()][:10]
            except Exception as ex:
                logging.error(f"Failed to parse legends: {ex}")
                mass_vals = []
            ...
            for idx, m in enumerate(mass_vals, start=1):
                try:
                    ...
                    logging.warning(f"Failed writing mass{idx}: {ex}")
            ...
            while self._running:
                try:
                    ...
                    # attempt to recover by reinitializing client
                    try:
                    ...
                    try:
                        ...
                        logging.error(f"Reinitialize after data error failed: {ex2}")
                    await asyncio.sleep(1.0)
                    continue

                if data:
                    row = data[-1]  # use last cycle
                    if mass_vals and len(row) >= len(mass_vals):
                        for idx, val in enumerate(row, start=1):
                            try:
                                await getattr(self, f'mid{idx}').write(val)
                            except Exception as ex:
                                logging.warning(f"Failed writing mid{idx}: {ex}")
